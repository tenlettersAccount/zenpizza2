$(document).ready(function () {
    ...
    //function search
    function usin(position_u, radius_u, width_u, height_u){
        $('.usearch').css({
            marginTop: position_u,

        });
        $('.usin').css({
            //height: height_u+'px',
            borderRadius: radius_u +'px'
        });
        var search_pos = $('.usearch').offset();
        //console.log(search_pos);
        $('.selectblog').css('top', search_pos.top+height_u+9);
    ...
    if(navigator.platform.toUpperCase().indexOf('MAC')>=0){ $('.fa-search').css({top: '-0px'}); }

    /* change slider h2 paddingtop if screen width val is higher then usual */
    if(w > 1500){ $('.mainh2d').css({paddingTop: '20px'}); }
    if(w >= 1821){ $('.mainh2d').css({paddingTop: '40px'}
